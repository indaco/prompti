...
// -----------------------------------------------------------------------
// Inline-mode model tests
// -----------------------------------------------------------------------

func testInlineModel() model {
	cfg := &Config{
		Mode:     ModeInline,
		Question: "Continue?",
	}
...
func TestInline_InitialConfirmation_IsTrue(t *testing.T) {
	m := testInlineModel()
	if !m.confirmation {
		t.Error("expected initial confirmation to be true")
	}
...
func TestInline_Update_Y_SetsConfirmationTrue_Quitting(t *testing.T) {
	m := testInlineModel()
	m.confirmation = false // start with false to verify "y" sets it true
	msg := keyPress('y', 0, "y")
	updated, cmd := m.Update(msg)
...
func TestInline_Update_N_SetsConfirmationFalse_Quitting(t *testing.T) {
	m := testInlineModel()
	msg := keyPress('n', 0, "n")
	updated, cmd := m.Update(msg)
	um := updated.(model)
...
func TestInline_Update_Left_TogglesConfirmation(t *testing.T) {
	m := testInlineModel() // confirmation starts true
	msg := keyPress(tea.KeyLeft, 0, "")
	updated, _ := m.Update(msg)
	um := updated.(model)
...
func TestInline_Update_Right_TogglesConfirmation(t *testing.T) {
	m := testInlineModel() // confirmation starts true
	msg := keyPress(tea.KeyRight, 0, "")
	updated, _ := m.Update(msg)
	um := updated.(model)
...
func TestInline_Update_Enter_QuitsWithCurrentState(t *testing.T) {
	m := testInlineModel()
...
func TestInline_Update_CtrlC_SetsQuitting(t *testing.T) {
	m := testInlineModel()
	msg := keyPress('c', tea.ModCtrl, "")
	updated, cmd := m.Update(msg)
	um := updated.(model)
...
func TestInline_Update_Esc_SetsQuitting(t *testing.T) {
	m := testInlineModel()
	msg := keyPress(tea.KeyEscape, 0, "")
	updated, cmd := m.Update(msg)
	um := updated.(model)
...
func TestInline_View_Quitting_ReturnsEmpty(t *testing.T) {
	m := testInlineModel()
	m.quitting = true
	v := m.View()
...
func TestInline_View_NotQuitting_ReturnsNonEmpty(t *testing.T) {
	m := testInlineModel()
	v := m.View()
